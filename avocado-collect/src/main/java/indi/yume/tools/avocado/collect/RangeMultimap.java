...
public class RangeMultimap<K extends Comparable, V> implements Map<Range<K>, V> {
    ...
    @EqualsAndHashCode
    private static final class RangeMapEntry<K extends Comparable, V> implements Entry<Range<K>, V> {
    ...
    public Set<Entry<Range<K>, V>> getByPoint(K point) {
        ...
        HashSet<Entry<Range<K>, V>> resultSet = new HashSet<>();
    ...
    public void putAll(Map<? extends Range<K>, ? extends V> map) {
        for(Entry<? extends Range<K>, ? extends V> entry : map.entrySet())
    ...
    public Collection<Entry<Range<K>, V>> entries() {
        return allValueMap.entries();
    }
